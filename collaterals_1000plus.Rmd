...
# load data
allpts <- read.table("all_data_v2.csv", header = T, sep=",")
...
model_8 <- lmrob(infarct_growth_D5FLAIR~Rekan*reperf + NIHSS_ad + iv_tPR + HIR1*OCC_LOC_1, data = allpts)
...
model_9 <- lmrob(DWIgrowth~Rekan*reperf + NIHSS_ad + iv_tPR + HIR1*OCC_LOC_1, data = allpts)
...
stargazer(model_6, model_7, model_8, model_9, type="html", align = F, style="all", single.row=T)
...
model_10 <- glm(infarct_growth~Rekan*reperf*OCC_LOC_1,family = binomial(link="logit"), data=allpts)
#plot(allEffects(model_9), multiline=T, ci.style="bars")
stargazer(model_10, type="html", align = F, style="all", single.row=T)
```
