...
# descriptive stats
#allpts_summ <- allpts[,c(2:12,15:19,28:33)] # define variables for summary
...
higa <- c()
higa$val <- c(allpts$higashida_D1, allpts$higashida_D2)
higa$day <- c(rep(1,length(allpts$higashida_D1)), rep(2,length(allpts$higashida_D2)))
#plot(factor(higa$day), factor(higa$val), legend=("topright", legend=c("0","1","2","3","4")))
```
<style>
...
```{r, echo= FALSE, fig.width=5, fig.height=5, fig.align='center',dpi=300, warning=F, results="asis"}
...
#### Predictors of extent of reperfusion in recanalizers
```{r, echo= FALSE, fig.width=5, fig.height=5, fig.align='center',dpi=300, warning=F, results="asis"}
...
model_A <- lmrob(mRS_d90~NIHSS_ad + iv_tPR, data=allpts, setting = "KS2014")
...
model_B <- lmrob(mRS_d90~NIHSS_ad + iv_tPR + Rekan + Tmax_6_reperf_rel, data=allpts, setting = "KS2014")
...
model_C <- lmrob(mRS_d90~NIHSS_ad + iv_tPR  + Tmax_6_reperf_rel + HIR1 + Rekan, data=allpts, setting = "KS2014")
boxplot(allpts$HIR1~allpts$mRS_d90, xlab="mRS (day 90)", ylab="HIR")

model_D <- lmrob(mRS_d90~NIHSS_ad + iv_tPR  + Tmax_6_reperf_rel + higashida_D1_bin + Rekan, data=allpts, setting = "KS2014")
...
```{r, echo= FALSE, fig.width=5, fig.height=5, fig.align='center',dpi=300, warning=F, results="asis"}
model_A <- lmrob(DWIgrowth~NIHSS_ad + iv_tPR + Tmax_6_reperf_rel, data=allpts, setting = "KS2014")
...
model_B <- lmrob(DWIgrowth~NIHSS_ad + iv_tPR + Rekan*Tmax_6_reperf_rel, data=allpts, setting = "KS2014")
boxplot(DWIgrowth~Rekan, data=allpts, ylab="Absolute infarct growth (D1 - D2)", outline=OL, xlab="Recanalization")

model_C <- lmrob(DWIgrowth~NIHSS_ad + iv_tPR  + Tmax_6_reperf_rel + HIR1*Rekan, data=allpts, setting = "KS2014")
plot(allpts$HIR1,allpts$DWIgrowth, xlab="HIR", ylab="Absolute infarct growth (D1 - D2)")

model_D <- lmrob(DWIgrowth~NIHSS_ad + iv_tPR  + Tmax_6_reperf_rel + higashida_D1_bin*Rekan, data=allpts, setting = "KS2014")
...
```{r, echo= FALSE, fig.width=5, fig.height=5, fig.align='center',dpi=300, warning=F, results="asis"}
model_A <- lmrob(infarct_growth_D5FLAIR~NIHSS_ad + iv_tPR  + Tmax_6_reperf_rel, data=allpts, setting="KS2014")
...
model_B <- lmrob(infarct_growth_D5FLAIR~NIHSS_ad + iv_tPR + Rekan* + Tmax_6_reperf_rel, data=allpts, setting="KS2014")
boxplot(infarct_growth_D5FLAIR~Rekan, data=allpts, ylab="Absolute infarct growth (D1 - D5)", outline=OL, xlab="Recanalization")

model_C <- lmrob(infarct_growth_D5FLAIR~NIHSS_ad + iv_tPR  + Tmax_6_reperf_rel + HIR1*Rekan, data=allpts, setting="KS2014")
plot(allpts$HIR1,allpts$infarct_growth_D5FLAIR, xlab="HIR", ylab="Absolute infarct growth (D1 - D5)")

model_D <- lmrob(infarct_growth_D5FLAIR~NIHSS_ad + iv_tPR  + Tmax_6_reperf_rel + higashida_D1_bin*Rekan, data=allpts, setting="KS2014")
...
model_A <- glmrob(higashida_D1_bin~NIHSS_ad, data=allpts, family = "binomial")
model_B <- glmrob(higashida_D1_bin~NIHSS_ad + HIR1, data=allpts, family = "binomial")
...
stargazer(model_A, model_B, type="html", align = F, style="all", single.row=T)
stargazer(anova(model_A,model_B), type="html", align = F, style="all", single.row=T, title = "1 vs 2")
```

### Predictors of collateral flow defined by HIR
```{r, echo= FALSE, fig.width=8, fig.height=5, fig.align='center',dpi=300, warning=F, results="asis"}
model_A <- lmrob(HIR1~NIHSS_ad, data=allpts, setting = "KS2014")
model_B <- lmrob(HIR1~NIHSS_ad + higashida_D1_bin, data=allpts, setting = "KS2014")
