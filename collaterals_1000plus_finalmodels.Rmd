...
Binarized **infarct growth** is defined as absolute lesion growth (FLAIR D5 - DWI D0) >7mL.  
...
*Campbell BC V, Christensen S, Tress BM, Churilov L, Desmond PM, Parsons MW, Alan Barber P, Levi CR, Bladin C, Donnan GA, Davis SM (2013) Failure of collateral blood flow is associated with infarct growth in ischemic stroke. J Cereb Blood Flow Metab 33:1168–1172.*
...
#model_E <- lmrob(mRS_d90~NIHSS_ad + iv_tPR + higashida_change, data=allpts)
...
#model_E <- lmrob(rel_DWIgrowth~NIHSS_ad + iv_tPR + higashida_change, data=allpts)
#plot(allEffects(model_E), multiline=T, ci.style="bars")
...
#model_E <- lmrob(DWIgrowth~NIHSS_ad + iv_tPR + higashida_change, data=allpts)
#plot(allEffects(model_E), multiline=T, ci.style="bars")
...
#model_E <- lmrob(infarct_growth_D5FLAIR~NIHSS_ad + iv_tPR + higashida_change, data=allpts)
#plot(allEffects(model_E), multiline=T, ci.style="bars")
